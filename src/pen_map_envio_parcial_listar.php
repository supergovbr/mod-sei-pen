    ...
    try {
      ...
      if (len > 0) {
        if (confirm('Confirma a exclusão de ' + len + ' mapeamento(s) ?')) {
          ...
          form.attr('action', '<?php print $objSessaoSEI->assinarLink('controlador.php?acao=pen_map_envio_parcial_excluir&acao_origem=' . $acaoOrigem . '&acao_retorno=pen_map_envio_parcial_listar'); ?>');
          form.submit();
        }
      } else {
...
$nomeEstrutura=$_POST['nome_estrutura'];
...
<form id="frmAcompanharEstadoProcesso" method="post" action="">
  ...
  <input type="text" name="nome_estrutura" id="txtNomeEstrutura" class="infraText input-field-first" onkeyup="return tratarEnter(event)" value="<?php echo $nomeEstrutura; ?>" />

  <label for="nome_unidade" class="infraLabelOpcional input-label-second">Nome da Unidade:</label>
  <input type="text" name="nome_unidade" class="infraText input-field-second" onkeyup="return tratarEnter(event)" value="<?php echo $nomeUnidade; ?>" />
